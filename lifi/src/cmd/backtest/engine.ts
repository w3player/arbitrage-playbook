...
export class BacktestEngine {
  ...
  private consider(frame: OpportunityFrame): void {
    ...
    const maximumTrade = mulDiv(
      totalInitialWeth,
      BigInt(this.config.backtest.maxTradeInventoryBps),
      10_000n,
    )
  ...
  private recoverBySelling(chainId: number, amount: bigint, fromMs: number): { succeeded: boolean; atMs?: number; cost: bigint } {
  ...
  private recoverByBuying(chainId: number, amount: bigint, fromMs: number): { succeeded: boolean; atMs?: number; cost: bigint } {
    const quote = this.quoteIndex.findFirstSameChain(
      'buy-exact-output',
      chainId,
      amount,
