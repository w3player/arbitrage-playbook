...
const gas =
  (factor: bigint): QuoteTransform =>
  (quotes) =>
    quotes.map((quote) => ({ ...quote, gasUsdMicros: quote.gasUsdMicros * factor }));

const delay =
  (factor: number): ConfigTransform =>
  (config) => {
    config.backtest.executionDelayMs *= factor;
  };

const wethShock =
  (bps: bigint): FrameTransform =>
  (frames) =>
...
const scenarios: Scenario[] = [
  ...
  { id: 'delay-5x', name: '执行延迟 5 倍', config: delay(5) },
  {
    ...
    quotes: (quotes) =>
      quotes.map((quote) =>
        quote.kind === 'buy-exact-output'
          ? { ...quote, fromAmount: mulDiv(quote.fromAmount, 10_020n, 10_000n) + 1n }
          : quote.kind === 'sell-exact-input'
            ? {
                ...quote,
                toAmount: mulDiv(quote.toAmount, 9_980n, 10_000n),
                toAmountMin: mulDiv(quote.toAmountMin, 9_980n, 10_000n),
              }
            : quote,
      ),
  },
  {
    id: 'api-gaps',
    name: '规律漏掉三分之一报价',
    quotes: (quotes) => quotes.filter((_quote, index) => index % 3 !== 2),
  },
  {
    ...
    quotes: (quotes, config) => {
      ...
      return quotes.filter(
        (quote) =>
          Math.abs(quote.receivedAtMs - middle) > 30 * 60_000 ||
  ...
  },
  {
    ...
    name: '买卖各 5% 失败',
    config: (config) => {
      config.backtest.failure = { correlatedFailureBps: 100, buyFailureBps: 500, sellFailureBps: 500 };
    },
  },
  {
    id: 'bridge-slow-5x',
    name: '跨链耗时 5 倍',
    config: (config) => {
      config.backtest.rebalance.defaultDurationMs *= 5;
    },
    quotes: (quotes) =>
      quotes.map((quote) =>
        quote.kind === 'bridge-exact-input' ? { ...quote, executionDurationMs: quote.executionDurationMs * 5 } : quote,
      ),
  },
  {
    id: 'bridge-anomalies',
    name: '跨链异常概率提高',
    config: (config) =>
      Object.assign(config.backtest.rebalance, {
        completedBps: 8500,
        refundedBps: 700,
        partialBps: 500,
        failedBps: 300,
  ...
  },
  {
    ...
    frames: (frames) => {
      ...
      return first
        ? frames.filter((frame) => frame.buyChainId === first.buyChainId && frame.sellChainId === first.sellChainId)
        : frames;
    },
  },
